...
from agent_service.utils.sec.constants import FILE_10K, FILE_10Q, FILE_20F
...
class TestSecFilings(TestExecutionPlanner):
    ...
    @skip_in_ci
    def test_get_20f_sec_filings(self) -> None:
        # tests retrieving SEC filings for foreign company (20-F)

        prompt = "Summarize SEC filings for the last year from SONY"
            ...
            self.assertTrue(
                len(
                    [
                        ...
                        for sec_filing in args["texts"]
                        if isinstance(sec_filing, StockOtherSecFilingText)
                        and (sec_filing.form_type == FILE_20F)
                    ]
                )
                >= 0
            )

...
@tool(
    description=(
        ...
        "If there is a target stock and you are filtering to get its peers as `stocks`, and the client does "
        "not mention an stock index or country to use as the pool of potential competitors, you should "
        "filter its peers from a popular stock index that contains the stock, e.g. if the stock was "
        "Carrefour you might choose the Stoxx 600 as your default stock universe if the client says "
        "nothing else about which competitors to use, since Carrefour is a European stock on the "
        "Stoxx 600. However, anything the client says about potential competitors takes precedence!"

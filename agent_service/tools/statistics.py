...
@tool(
    description=(
        ...
        " 'for each of N years/months'. If the client says something like that, read these instructions carefully!"
        " In the these case you must ALSO include a date_range, generally it is very rare to be missing a date_range"
        ...
        " 6 months) in the statistic_reference."
...
async def get_statistic_data_for_companies(
    ...
    if len(stat_list) == 0:
        ...
        if any([stat not in all_statistic_lookup for stat in stat_list]):
            logger.warning(f"One of statistics in {stat_list} is not a supported stat, retrying")
            result = await llm.do_chat_w_sys_prompt(main_prompt, DECOMPOSITION_SYS_PROMPT.format())
            output_json = json.loads(clean_to_json_if_needed(result))
            stat_list = output_json["components"]

...
UPDATE_TIMESPAN_MAIN_PROMPT = Prompt(
    name="UPDATE_TIMESPAN_MAIN_PROMPT", template=UPDATE_TIMESPAN_MAIN_PROMPT_STR
)
            ...
            if prev_other:
                ...
                if not bad_stat:
                    ...
                    if old_decomp_description == "" or (
                        ...
                        if new_calculation != old_decomp_description:
                            logger.info("Updated calculation description")

                        if old_decomp["extra_timespan"]:
                            update_timespan_prompt = UPDATE_TIMESPAN_MAIN_PROMPT.format(
                                statistic_reference=args.statistic_reference,
                                old_decomp_description=old_decomp_description,
                                new_decomp_description=new_calculation,
                                old_timespan=old_decomp["extra_timespan"],
                            )
                            new_timespan = await llm.do_chat_w_sys_prompt(
                                update_timespan_prompt, NO_PROMPT
                            )
                            if new_timespan != old_decomp["extra_timespan"]:
                                logger.info(f"Updated extra timespan to '{new_timespan}'")

                    decomp_json = {
                        "extra_timespan": new_timespan,

...
RELEVANCY_INSTRUCTION = """For the company {company_name}, identify relevant kpis around the following topic: '{query_topic}'. Limit yourself to at most 20 KPIs. You should not aim to get 20, stop when you feel that you have identified All relevant KPIs to the topic, however if you feel there are more than 20, return the 20 most impactful KPIs."""

SPECIFIC_KPI_INSTRUCTION = """For the company {company_name}, identify a single KPI that is most relevant to the topic '{query_topic}'. If there are multiple KPIs that are relevant, choose the one that is most impactful to the company."""

KPI_RELEVANCY_MAIN_PROMPT = """"{instructions} If you are unsure or there is not enough information given, defer to KPIs that measure revenue/sales and take the ones with the highest amount.
...
Return the relevant KPIs by name and the numerical identifier associated with it. Each line will contain a relevant KPI with the KPI number without the brackets followed by the KPI name separated by a comma. Do not output any additional explanation or justification. Do not output more than 10 lines."""

...
- Before each line of code, you will have a comment line which first has the one function you will use, and then each of the arguments to the function which must follow all the requirements, i.e. not containing functions, indexes, or operators. Do not include the keywords, just the argument values. If there is not one function, or one of the argument violates one of the above requirements, write another comment line which fixes the problems before you write the next line of code.
...
- Only the output returned by the last line of the script will be shown to the user, so you must be sure that it has all the information the user is asking for"""

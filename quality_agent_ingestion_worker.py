...
from agent_service.sqs_serve.horizon_message_handler import HorizonMessageHandler
...
async def poll_sqs_forever() -> None:
    ...
    queue_url = AGENT_QUALITY_WORKER_QUEUE
    message_handler = HorizonMessageHandler()

    if not queue_url:
        raise Exception("Environment variable AGENT_QUALITY_WORKER_QUEUE must be set.")
    ...
    while not graceful_sigterm.kill_now:
        ...
        for message in messages["Messages"]:
            ...
            try:
                ...
                log_event(
                    event_name="agent_quality_worker_message_processed",
                    event_data={
